...
class Car(pygame.sprite.Sprite):
    def __init__(self, world, coordinate: tuple, car_no: int, sensor_num, angle: int):
        ...
        self.origin_image = pygame.transform.scale(
            pygame.image.load(
                path.join(ASSET_IMAGE_DIR, f"car_0{self.car_no+1}.png")),
            self.size)
        ...
        self.box = self.body.CreatePolygonFixture(
            box=(1, 1), density=1, friction=0.1, restitution=0.3)
    ...
    def update(self, commands):
        ...
        self.image = pygame.transform.rotate(
            self.origin_image, (self.body.angle * 180 / math.pi) % 360)
        self.rect = self.image.get_rect()
    ...
    def left_move(self, pwm: int):
        if pwm < 0:
            self.sensor.sensor_left.linearVelocity = self.body.GetWorldVector(
                localVector=(0, -(abs(pwm) ** 0.5)))
        else:
            self.sensor.sensor_left.linearVelocity = self.body.GetWorldVector(
                localVector=(0, pwm ** 0.5))

    def right_move(self, pwm: int):
        if pwm < 0:
            self.sensor.sensor_right.linearVelocity = self.body.GetWorldVector(
                localVector=(0, -(abs(pwm) ** 0.5)))
        else:
            self.sensor.sensor_right.linearVelocity = self.body.GetWorldVector(
                localVector=(0, pwm ** 0.5))
