...
def create_dqn():
    ...
    nn.compile(loss='mse', optimizer=Adam(learning_rate=ALPHA))
    return nn
...
class DoubleDQNAgent(object):
    ...
    def act(self, state):
        if self.epsilon > np.random.rand():
            ...
            q_values = self.online_network.predict(state, verbose=0)[0]
    ...
    def experience_replay(self):
        ...
        for experience in minibatch:
            ...
            state = self._reshape_state_for_net(state)
            experience_new_q_values = self.online_network.predict(state, verbose=0)[
                0]
            if done:
                q_update = reward
            else:
                ...
                online_net_selected_action = np.argmax(
                    self.online_network.predict(next_state, verbose=0))
                # using target network to EVALUATE action
                target_net_evaluated_q_value = self.target_network.predict(
                    next_state, verbose=0)[0][online_net_selected_action]
        ...
        self.online_network.fit(
            minibatch_states, minibatch_new_q_values, epochs=1)
...
def test_agent():
    ...
    for episode_index in range(1, MAX_TRAINING_EPISODES+1):
        ...
        print('E %d scored %d, avg %.2f' %
              (episode_index, episode_score, last_100_avg))
        if len(trial_episode_scores) >= 100 and last_100_avg >= 195.0:
            print('Trial solved in %d episodes!' %
                  ((episode_index - 100)))
            break
...
def plot_trials(trials):
    ...
    for i, trial in enumerate(trials):
        ...
        plt.bar(np.arange(i, i+1), steps_till_solve, 0.5,
                color=bar_color, align='center', alpha=0.5)
